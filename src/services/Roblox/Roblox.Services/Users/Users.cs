...
public class UsersService : ServiceBase, IService
{
    ...
    public async Task<bool> IsUserApproved(long userId)
    {
        ...
        // Default
        return true; // insecure, but who cares?
        // if anyone has a proper fix plz pull request
    }
    ...
    public async Task CreateInvite(long authorUserId)
    {
        ...
        if (floodChecked && authorUserId != 1 && authorUserId != 2)
            throw new RobloxException(403, 0, "Too many invites created. Try again tomorrow.");
#endif
        var canCreate = authorUserId == 1 || authorUserId == 2 || await CanCreateInvite(authorUserId);
        if (!canCreate)
            throw new RobloxException(401, 0, "Unauthorized");
        var userInfo = await GetUserById(authorUserId);
        if (authorUserId != 1 && authorUserId != 2 && userInfo.created > DateTime.UtcNow.Subtract(TimeSpan.FromDays(1)))
